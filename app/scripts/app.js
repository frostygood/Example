	...
	$.fn.validateForm = function (config) {//создаем виджет
		...
		$(this).on('checkInput keyup', function(_event) { //в константу запихнули валидатор, чтобы его нельзя было изменить, и не вызывать когда нужно
			...
			if (('validEmail' in config) && (config.validEmail.value == true)) { //если в конфиге есть свойство валидЭмейл, то
				var emails = /^[\w\-\.]{1,64}@[\w\-]{1,253}\.[\w\-]{2,30}$/gim; //начало строки, английские буквы от одной до бесконечности + разрешенные символы, собака, опять буквы от одной до бесконечности + разрешенные символы, точка, буквы от двух до четырёх (если почта где-нибудь на .info), конец строки. Регистр букв не учитывается
